...
- Flutter：`flutter analyze` 0 条诊断，78 项测试通过，Android debug APK 与 iOS Simulator debug 构建通过。
...
- Flutter 已接入账号级增量拉取、严格密文校验、游标、SSE/前台触发与本地密文变更缓存；CMK 生成/封装、受信设备批准、恢复密钥、加密出站队列及解密落库仍未接入，因此不能把“新设备自动恢复全部历史内容”标记为端到端交付。
