...
| AI 与 Codex | 后台设置页配置第三方 API/Key、自动发现 Responses/生图模型、SSE/JSON 兼容、Responses 工具循环、生图、递归 schema 校验、风险审批；官方 Codex App Server JSON-RPC、SSH Agent transport、动态工具、审批/MCP elicitation 默认拒绝 |
...
- Flutter：`flutter analyze` 0 条诊断，30 项测试通过。Flutter 3.44 的 analysis server 在中文绝对路径上会截断 LSP JSON，本轮在同源码的 ASCII 临时副本复验通过。
