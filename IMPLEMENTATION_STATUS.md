...
| 端到端密钥初始化与恢复 | Rust 用系统 CSPRNG 生成 256 位 CMK 与高熵恢复密钥；CMK 只进入设备密钥 AEAD 保护、原子写入且权限为 `0700/0600` 的账号级 `vault.db`，后端只保存 HKDF-SHA256 + AES-256-GCM 恢复密钥信封；并发初始化采用首写胜出且冲突时丢弃本地待确认密钥；Flutter 已实现恢复密钥展示、复制限时清理、后台遮挡和二次确认；新设备只下载加密信封，恢复密钥在 Rust 本地以账号绑定 AAD 解封；受信设备端已接入 10 分钟一次性批准/拒绝页，验证码绑定账号、请求与 X25519 公钥，CMK 只以 X25519 + HKDF-SHA256 + AES-256-GCM 定向封装给请求设备；错误、篡改、过期或跨账号材料不会写入内容密钥 |
...
- Flutter：`flutter analyze` 0 条诊断，118 项测试通过，Android debug APK 与 iOS Simulator debug 构建通过。
...
- Flutter 已接入账号级增量密文拉取、恢复密钥展示/确认、新设备恢复密钥解锁和受信设备端批准/拒绝，Rust `vault.db` 已实现 CMK 初始化、恢复以及面向请求设备公钥的 CMK 封装/解封，服务端只中继账号隔离的公钥和密文状态。新设备发起/等待批准页面、批准结果本机落地编排、加密出站队列、解密落库及历史内容重建仍未全部接入，因此不能把“新设备自动恢复全部历史内容”标记为端到端交付。
