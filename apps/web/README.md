...
- `/admin`：ChatGPT 登录保护的 AI Provider 配置、连通测试、生图、投票创建与设备令牌签发/撤销。
...
运行变量只有 `AI_SECRET_MASTER_KEY`（32 字节随机值的 Base64）。只通过本地
`.env` 或 Sites Secret 注入，不写入仓库。移动端令牌从后台签发，数据库只保存
