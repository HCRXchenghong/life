...
# Android links call this file as a wrapper. Forward straight to the NDK linker
# instead of starting the Dart build tool for every final link invocation.
if [[ -n "${_CARGOKIT_NDK_LINK_CLANG:-}" && -n "${_CARGOKIT_NDK_LINK_TARGET:-}" ]]; then
  exec "$_CARGOKIT_NDK_LINK_CLANG" "$_CARGOKIT_NDK_LINK_TARGET" "$@"
fi

BASEDIR=$(dirname "$0")

# Cargokit selects Cargo through rustup. Keep rustc/rustdoc on that same
# toolchain even when standalone Homebrew binaries appear earlier in PATH.
if command -v rustup >/dev/null 2>&1; then
  export RUSTC="$(rustup which rustc --toolchain stable)"
  export RUSTDOC="$(rustup which rustdoc --toolchain stable)"
fi

# NDK LLD otherwise uses every host core, which creates a large peak while
# resolving Rust's statically linked SSH/crypto dependency graph.
if [[ -n "${CARGOKIT_NDK_VERSION:-}" ]]; then
  ANDROID_LINK_FLAGS=$'-C\x1flink-arg=-Wl,--threads=1'
  if [[ -n "${CARGO_ENCODED_RUSTFLAGS:-}" ]]; then
    export CARGO_ENCODED_RUSTFLAGS="${CARGO_ENCODED_RUSTFLAGS}"$'\x1f'"${ANDROID_LINK_FLAGS}"
  else
    export CARGO_ENCODED_RUSTFLAGS="${ANDROID_LINK_FLAGS}"
  fi
fi
