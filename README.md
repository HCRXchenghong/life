...
- 管理员停用 App 账号或重置密码时，后端会原子撤销该账号会话和 AI 短期凭证，并通过账号隔离 SSE 让前台 App 立即清理凭证、关闭账号运行时并返回登录态；恢复前台时还会主动复核会话。
...
- Plus/Pro 在请求前保守预占 token，完成后仅按上游 `usage` 的真实总 token 回填，同时记录输入、输出、缓存输入和推理 token，失败或无可信 usage 时释放预占；Max 仅免除用量额度，仍保留鉴权、超时和防滥用速率保护。
